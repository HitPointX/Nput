...
function qparam(key) { return params.get(key); }
// ── Globals ──────────────────────────────────────────────────
let layout = null;
...
const kbHighlights = new Map();
const btnHighlights = new Map();
...
async function fetchLayout() {
    ...
    if (!res.ok)
        throw new Error(`layout.json → ${res.status}`);
    return res.json();
}
// ── Dark mode ────────────────────────────────────────────────
...
// ── Highlight helpers ────────────────────────────────────────
function syncHighlights(map, active, now) {
...
function connect() {
    ...
    ws.addEventListener('message', (event) => {
        ...
        try {
            ...
            console.error('[nput] Bad JSON:', event.data);
            return;
        }
    ...
    ws.addEventListener('error', (e) => console.error('[nput] WS error:', e));
}
// ── Mode toggle ──────────────────────────────────────────────
if (modeBtn) {
